...
commands = ['Insecure Restart', 'JVM', 'rec_rec']
...
def add_labels(ax, rects, unit):
    for rect in rects:
        ...
        ax.annotate(f'{height:.3f} {unit}' if height < 10 else f'{height:.1f} {unit}',
                    ...
                    ha='center', va='bottom', fontsize=14)  # Larger font for bar labels
...
with PdfPages('comparison_graphs.pdf') as pdf:
    ...
    ax1.set_xlabel('System', fontsize=14)  # Larger font for x-axis label
    ax1.set_ylabel('Average Time (ms)', fontsize=14)  # Larger font for y-axis label
    ax1.set_xticks(x)
    ax1.set_xticklabels(sorted_commands, fontsize=14)  # Larger font for tick labels
    ...
    ax2.set_xlabel('System', fontsize=14)  # Larger font for x-axis label
    ax2.set_ylabel('Average Memory Usage (MB)', fontsize=14)  # Larger font for y-axis label
    ax2.set_xticks(x)
    ax2.set_xticklabels(sorted_commands, fontsize=14)  # Larger font for tick labels
